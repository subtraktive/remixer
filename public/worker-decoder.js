//import MohayonaoWavDecoder from './wavedecoder.js';
/* MohayonaoWavDecoder
 * 
...
MohayonaoWavDecoder.prototype.decodeChunkSync = function(arrayBuffer) {
    ...
    console.log("DECODIGN CHUNK", arrayBuffer)
...
MohayonaoWavDecoder.prototype._init = function(reader) {
    console.log(" READER STRING LOOKS LIKE ==================================", reader.string(4))
...
self.onmessage = event => {
    //console.log("MESSAGE RECIEVED IN WORKED > %%%%%%(#######((((((((((((((((((((((#")
    const { decode, sessionId, trackIndex } = event.data;
    //console.log("THE EVENT IS", event.data)
    if (decode) {

        const decoded = decoder.decodeChunkSync(event.data.decode);
        //console.log("==========================+RECEIVEING DECODE", decoded)
        self.postMessage({ decoded, sessionId, trackIndex }, [
